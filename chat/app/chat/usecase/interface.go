...
type GetMessageReadersUseCase interface {
	Execute(cfbrCtx *fiber.Ctx, ctx context.Context, messageID uuid.UUID) ([]domain.User, error)
}

type DeleteAllMessagesFromConversationUseCase interface {
	Execute(cfbrCtx *fiber.Ctx, ctx context.Context, conversationID uuid.UUID) error
}
type Hub interface {
